...
public class Main {
    public static void main(String[] args) {
        ...
        Skeleton skeleton = new Skeleton("Кристофер", 10, 1, 85, 1, 2, 1);
        Goblin goblin = new Goblin("Людвиг", 15, 6, 40, 1, 2, 1);
        ...
        while (isContinue) {
            ...
            switch (input) {
                ...
                case "2":
                    if (Math.random() > 0.5) {
                        monster = skeleton;
                        System.out.println("появляется скелет " + skeleton.getName());
                    } else {
                        monster = goblin;
                        System.out.println("появляется гоблин " + goblin.getName());
                    }
                    Battle battle = new Battle(hero, monster);
                    Thread thread = new Thread(battle);
                    thread.start();
                    scanner.nextLine();
                    if (hero.getLife() <= 0) isContinue = false;
                    break;
                case "3":
                    isContinue = false;
                    break;
